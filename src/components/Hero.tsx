"use client";

import { Link } from "@/i18n/routing";
import { motion } from "framer-motion";
// import Image from "next/image";
// import HeroBackgroundImage from "@/public/images/hero-background-2.jpg";
// import { Link } from "@/i18n/routing";
import { useTranslations } from "next-intl";
// import { Fragment } from "react";

const Hero = () => {
  const t = useTranslations();

  // return (
  //   <section className="relative h-screen flex items-center justify-center overflow-hidden">
  //     <Image
  //       src={HeroBackgroundImage}
  //       alt="Coffee brewing scene"
  //       layout="fill"
  //       objectFit="cover"
  //       quality={100}
  //       priority
  //     />
  //     {/* <div className="absolute inset-0 bg-black bg-opacity-50" /> */}
  //     <div className="relative z-10 text-center w-full">
  //       <motion.h1
  //         className="text-4xl md:text-6xl font-bold mb-4 text-main drop-shadow-lg max-w-[95%] md:max-w-[85%] lg:max-w-[75%] xl:max-w-[65%] mx-auto"
  //         initial={{ opacity: 0, y: 20 }}
  //         animate={{ opacity: 1, y: 0 }}
  //         transition={{ duration: 0.8 }}
  //       >
  //         {t("hero.title")}
  //       </motion.h1>
  //       <motion.p
  //         className="text-xl md:text-2xl mb-8 text-main/80 drop-shadow-md max-w-[90%] md:max-w-[80%] lg:max-w-[70%] xl:max-w-[60%] mx-auto"
  //         initial={{ opacity: 0, y: 20 }}
  //         animate={{ opacity: 1, y: 0 }}
  //         transition={{ duration: 0.8, delay: 0.2 }}
  //       >
  //         {t("hero.subtitle")}
  //       </motion.p>

  //       <motion.div
  //         className="flex flex-col sm:flex-row gap-4 mt-8 justify-center items-center"
  //         initial={{ opacity: 0, y: 20 }}
  //         animate={{ opacity: 1, y: 0 }}
  //         transition={{ duration: 0.8, delay: 0.6 }}
  //       >
  //         <Link
  //           href="https://apps.apple.com/uz/app/itv-kino-seriallar-va-tv/id1144347735"
  //           target="_blank"
  //           className="inline-flex items-center px-6 py-3 rounded-xl bg-black/80 hover:bg-black/90 backdrop-blur-sm transition-all duration-300 text-white border border-white/10 min-w-56 justify-center"
  //         >
  //           <svg
  //             className="w-8 h-8 mr-2"
  //             viewBox="0 0 24 24"
  //             fill="currentColor"
  //           >
  //             <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z" />
  //           </svg>
  //           <div className="flex flex-col font-sans">
  //             <span className="text-xs">{t("hero.download")}</span>
  //             <span className="text-xl font-semibold">App Store</span>
  //           </div>
  //         </Link>
  //         <Link
  //           href="https://play.google.com/store/apps/details?id=uz.i_tv.player&hl=en"
  //           className="inline-flex items-center px-6 py-3 rounded-xl bg-black/80 hover:bg-black/90 backdrop-blur-sm transition-all duration-300 text-white border border-white/10 min-w-56"
  //         >
  //           <svg
  //             className="w-8 h-8 mr-2"
  //             viewBox="0 0 24 24"
  //             fill="currentColor"
  //           >
  //             <path d="M3,20.5V3.5C3,2.91 3.34,2.39 3.84,2.15L13.69,12L3.84,21.85C3.34,21.6 3,21.09 3,20.5M16.81,15.12L6.05,21.34L14.54,12.85L16.81,15.12M20.16,10.81C20.5,11.08 20.75,11.5 20.75,12C20.75,12.5 20.5,12.92 20.16,13.19L17.89,14.5L15.39,12L17.89,9.5L20.16,10.81M6.05,2.66L16.81,8.88L14.54,11.15L6.05,2.66Z" />
  //           </svg>
  //           <div className="flex flex-col font-sans">
  //             <span className="text-xs">{t("hero.download")}</span>
  //             <span className="text-xl font-semibold">Google Play</span>
  //           </div>
  //         </Link>
  //       </motion.div>
  //     </div>
  //   </section>
  // );

  return (
    <div className="relative overflow-hidden w-full sm:min-h-screen">
      <div
        className="absolute inset-0 z-0 bg-center sm:bg-left"
        style={{
          backgroundImage: 'url("/images/hero-background-4.jpg")',
          backgroundSize: "cover",
        }}
      />
      {/* Gradient Overlay */}
      <div className="absolute inset-0 z-0 bg-gradient-to-l from-transparent via-white/95 to-white" />
      <div className="absolute inset-0 z-0 bg-gradient-to-b from-transparent via-white/20 to-white" />

      <div className="container mx-auto mt-10">
        <div className="relative z-10 flex min-h-[calc(100vh-80px)] items-center px-6 lg:px-12 w-full">
          <div className="max-w-3xl">
            <h1 className="text-4xl font-bold tracking-tight text-primary sm:text-6xl md:text-7xl">
              {t("hero.sweet")}
              <br />
              {t("hero.subscription")}
              <br />
              {t("hero.for-coffee")}
            </h1>
            <p className="mt-6 max-w-xl text-lg text-muted-foreground">
              {t("hero.subtitle")}
            </p>
            <motion.div
              className="flex flex-col sm:flex-row gap-4 mt-8 "
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.8, delay: 0.6 }}
            >
              <Link
                href="https://apps.apple.com/uz/app/itv-kino-seriallar-va-tv/id1144347735"
                target="_blank"
                className="inline-flex items-center px-6 py-3 rounded-xl bg-black/80 hover:bg-black/90 backdrop-blur-sm transition-all duration-300 text-white border border-white/10 min-w-56 justify-center"
              >
                <svg
                  className="w-8 h-8 mr-2"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                >
                  <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z" />
                </svg>
                <div className="flex flex-col font-sans">
                  <span className="text-xs">{t("hero.download")}</span>
                  <span className="text-xl font-semibold">App Store</span>
                </div>
              </Link>
              <Link
                href="https://play.google.com/store/apps/details?id=uz.i_tv.player&hl=en"
                className="inline-flex justify-center items-center px-6 py-3 rounded-xl bg-black/80 hover:bg-black/90 backdrop-blur-sm transition-all duration-300 text-white border border-white/10 min-w-56"
              >
                <svg
                  className="w-8 h-8 mr-2"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                >
                  <path d="M3,20.5V3.5C3,2.91 3.34,2.39 3.84,2.15L13.69,12L3.84,21.85C3.34,21.6 3,21.09 3,20.5M16.81,15.12L6.05,21.34L14.54,12.85L16.81,15.12M20.16,10.81C20.5,11.08 20.75,11.5 20.75,12C20.75,12.5 20.5,12.92 20.16,13.19L17.89,14.5L15.39,12L17.89,9.5L20.16,10.81M6.05,2.66L16.81,8.88L14.54,11.15L6.05,2.66Z" />
                </svg>
                <div className="flex flex-col font-sans">
                  <span className="text-xs">{t("hero.download")}</span>
                  <span className="text-xl font-semibold">Google Play</span>
                </div>
              </Link>
            </motion.div>
            {/* <div className="mt-8 flex gap-4">
              <button className="bg-primary text-white px-4 py-2 rounded-md  hover:text-cream transition-colors">
                Our Services
              </button>
              <button className="bg-primary text-white px-4 py-2 rounded-md  hover:text-cream transition-colors">
                Learn More
              </button>
            </div> */}
          </div>
        </div>
      </div>
    </div>
  );
};

export default Hero;
